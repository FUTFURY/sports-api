...
function isCyberLeague(item) {
    ...
    return Boolean(
        ...
        name.includes('ebasketball') || 
        name.includes('virtual')
    );
}

function parseLeaguesFromFeed(data, targetSportId = null, includeCyber = false) {
    ...
    for (const item of data.Value) {
        ...
        if (item.LI) {
            ...
            if (!includeCyber && isCyberLeague(item)) continue;
        ...
        // Case B: item contains an array of leagues in item.L
            ...
            for (const sub of item.L) {
                if (sub.LI) {
                    ...
                    if (targetSportId && String(sportId) !== String(targetSportId)) continue;
                    if (!includeCyber && isCyberLeague(sub)) continue;
...
/**
 * Fetches clean match data (past and upcoming) for a whole CHAMPIONSHIP/LEAGUE.
 */
export const fetchChampionshipDetailedStats = async (hexId, lang = 'fr') => {
    const cacheKey = `champ_stats_detailed_${hexId}_${lang}`;
    const cached = cache.get(cacheKey);
    if (cached) return cached;
    ...
    try {
        ...
        return { name: null, upcoming: [], past: [] };
    }
};
